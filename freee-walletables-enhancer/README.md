...
通常、口座の一覧画面では現在の**登録残高**と**同期残高**が表示されますが、決算時にはそれらの残高よりも**期末残高**を知りたいので、あわせて表示するようにしています。

また、たとえば「Amazonビジネス」や「モバイルSuica」のような口座が決算書上ではどの勘定科目に対応するのか、口座の一覧画面と詳細画面ですぐ確認できるようになります。  
口座に紐づく仕訳を総勘定元帳で確認したいときには、総勘定元帳を開いて検索するという操作をしなくても、口座の一覧画面と詳細画面からワンクリックで開けるようになります。

## スクリーンショット

|▼口座の一覧画面|
|:---|
|![「口座一覧をコピー」のボタン、勘定科目、期末残高の表示と、総勘定元帳へのリンクが追加される](./screenshot_1.png)|

|▼ボタンのメニュー|
|:---|
|![「口座一覧をコピー」ボタンをクリックするとメニューが表示される](./screenshot_2.png)|

|▼現在の残高（リスト形式）|
|:---|
|![口座一覧（現在の残高）をコピーしてGoogleドキュメントに貼り付けたところ](./screenshot_3.png)|

|▼期末残高のみ（リスト形式）|
|:---|
|![口座一覧（期末残高のみ）をコピーしてGoogleドキュメントに貼り付けたところ](./screenshot_4.png)|

|▼全項目（タブ区切り）|
|:---|
|![口座一覧（タブ区切り）をコピーしてGoogleスプレッドシートに貼り付けたところ](./screenshot_5.png)|

|▼口座の詳細画面（例：モバイルSuica）|
|:---|
|![決算書表示名・期末残高バッジと「総勘定元帳」ボタンが追加される](./screenshot_6.png)|
